...
function hash(image){
  const pixelsMap = utils.pixelsMap(image,8,8);
  const bytesMap = utils.desaturate(pixelsMap);
  const average = utils.average(bytesMap);
  const thresholdMap = utils.thresholdMap(bytesMap, average);
  return thresholdMap;
}
...
function compare(first, second){
  ...
  const distance = utils.hammingDistance(first, second);
...
const utils = {
  ...
  hammingDistance: function(bitsA, bitsB){
    let diffMask = (bitsA ^ bitsB).toString(2);
